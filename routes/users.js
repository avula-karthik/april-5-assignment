    ...
    usersModel.findAll().then(
        function (users) {
            ...
            if (flag) {
                res.status(409).json({
                    status: 0,
                    debug_date: 'user already existed',
                });
            } else {
                usersModel
                    .create({
    ...
    usersModel
        ...
        .then((status) => {
            if (status === 1) {
                res.status(200).json({
                    status: 1,
                    debug_date: 'updated successfully',
                });
            } else {
                res.status(404).json({
                    status: 0,
                    debug_date: 'record not found',
                });
            }
        })
        .catch((error) => {
    ...
    usersModel
        ...
        .then((status) => {
            if (status === 1) {
                res.status(200).json({
                    status: 1,
                    debug_date: 'deleted successfully',
                });
            } else {
                res.status(404).json({
                    status: 0,
...
router.get('/loggeduser', (req, res) => {
    if (req.session.isLoggedIn === 1) {
        ...
        res.json({ status: 0, debug_data: 'Please Login' });
    }
});
